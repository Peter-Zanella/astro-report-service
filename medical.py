...
def compute_lifespan(chart: Dict) -> Dict:
    ...
    dusht = [p for p in GRAHAS if h(p) in _DUSHTHANA]
    t = (f"Wohltäter in Kendras: {', '.join(DE[p] for p in ben_kendra) or 'keine'} · "
         ...
         f"Planeten in Dushthanas: {', '.join(DE[p] for p in dusht) or 'keine'}")
    m = "+" if len(ben_kendra) > len(mal_kendra) and len(dusht) <= 2 else \
        "−" if (len(dusht) >= 4 or (mal_kendra and not ben_kendra)) else "·"
