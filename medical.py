...
def compute_focus(chart: Dict, kap: Dict, top_n: int = 3) -> Dict:
    """Zweischichtige Zuordnung der Gesundheits-Schwerpunkte (Seminar-Methodik):

      Ebene 1 — HAUS:    KAP-gewichtete Besetzung (voll) und Aspekte (halb)
                         je Haus vom Lagna.
      Ebene 2 — ZEICHEN: dieselbe Wertung je Tierkreiszeichen AB WIDDER
                         (feste Kālapuruṣa-Melothesie, lagna-unabhängig).

    Beide Ebenen werden über die zwölf Kālapuruṣa-Regionen zusammengeführt
    (Region k ≙ Haus k ≙ k-tes Zeichen ab Widder). Tragen BEIDE Ebenen zu
    einer Region bei, gilt das klassisch als Verstärkung (Konvergenz) —
    z.B. belastetes 3. Haus UND belastete Zwillinge → Bronchien doppelt.
    """
    pls = chart.get("planets", {})
    houses = chart.get("houses", {}) or {}
    totals = kap["totals"]
    ...
    sign_score = {s: 0.0 for s in range(12)}
    sign_who: Dict[int, List[str]] = {s: [] for s in range(12)}
    ...
    # Zusammenführung: Region k = Haus k + Zeichen (k-1) ab Widder
    regions = []
    for k in range(1, 13):
        s_idx = k - 1
        total = house_score[k] + sign_score[s_idx]
        if total <= 0:
            continue
        regions.append({
            "region": k,
            "sign": SIGN_DE[SIGNS[s_idx]],
            "score": round(total, 1),
            "house_part": round(house_score[k], 1),
            "sign_part": round(sign_score[s_idx], 1),
            "house_who": house_who[k][:4],
            "sign_who": sign_who[s_idx][:4],
            "convergence": house_score[k] > 0 and sign_score[s_idx] > 0,
            "body": SIGN_BODY[SIGNS[s_idx]],
            "body_house": HOUSE_BODY.get(k, ""),
        })
...
def compute_lifespan(chart: Dict) -> Dict:
    ...
    # "Beschädigt" heisst klassisch AUCH: Übeltäter beim Mond (Konjunktion im
    # selben Zeichen) oder Übeltäter-Drishti auf den Mond — die Engine-
    # Affliktionsliste (Verbrennung, Gandānta, Knoten …) allein reicht dafür
    # nicht. Fundstelle: Mond in 8 mit Mars und Saturn-Aspekt wurde zuvor
    # fälschlich als »unbeschädigt« bezeichnet.
    mh, ma = h("Moon"), aff.get("Moon", [])
    good_h = mh in {3, 4, 5, 9, 10, 11}
    _m_occ = [p for p in _MALEFICS
              if p != "Moon" and h(p) == mh and mh] if mh else []
    _m_asp = [p for p in _MALEFICS
              if p not in _m_occ and mh
              and mh in _aspected_houses(p, h(p))]
    dmg_bits = []
    if _m_occ:
        dmg_bits.append("Konjunktion: " + ", ".join(DE[p] for p in _m_occ))
    if _m_asp:
        dmg_bits.append("Aspekt: " + ", ".join(DE[p] for p in _m_asp))
    if ma:
        dmg_bits.append("; ".join(ma))
    damaged = bool(_m_occ or _m_asp or ma)
    t = (f"Haus {mh} ({'günstig' if good_h else 'nicht ideal'}), Mond {moon_state}"
         + (f", beschädigt durch Übeltäter — {' · '.join(dmg_bits)}"
            if damaged else ", unbeschädigt"))
    m = ("+" if good_h and moon_state == "hell" and not damaged else
         "−" if (mh in _DUSHTHANA or mh in (1, 7))
                and (moon_state == "dunkel" or damaged)
         else "·")
    row("Stärke & Position des Mondes", t, m)

    # 3b. Balarishta — klassische Kindheits-Arishta-Kombinationen (BPHS).
    # Retrospektiv zu lesen (frühe Kindheitsjahre); zählt nur ohne Aufhebung
    # als Minus. Kern: Mond in 6/8/12 unter Übeltäter-Einfluss ohne
    # Wohltäter-Schutz; dazu Gandanta/Sandhi, Papakartari, enge Knotennähe,
    # dunkler Mond im Dushthana.
    _bal_find: List[str] = []
    _m_infl = [p for p in _influences_on_house(chart, mh) if p != "Moon"] if mh else []
    _m_mal = [p for p in _m_infl if p in _MALEFICS]
    _m_ben = [p for p in _m_infl if p in _BENEFICS]
    if mh in _DUSHTHANA and _m_mal:
        _bal_find.append(f"Mond in {mh} unter {', '.join(DE[p] for p in _m_mal)}")
    if moon_state == "dunkel" and mh in _DUSHTHANA:
        _bal_find.append(f"dunkler Mond im Dushthana ({mh})")
    for _a in ma:
        _al = _a.lower()
        if "gandanta" in _al or "sandhi" in _al:
            _bal_find.append(f"Mond: {_a}")
        elif "papakartari" in _al:
            _bal_find.append("Mond im Papakartari (Übeltäter-Zange)")
        elif "nodal" in _al or "grahana" in _al:
            _bal_find.append(f"Mond: {_a}")
    # Ariṣṭa-Bhaṅga (Aufhebungen). WICHTIG: Ein selbst affligierter Schützer
    # (Debilitation, Verbrennung, Knoten-Konjunktion, Graha Yuddha) gibt KEIN
    # volles Bhaṅga — er wird nur als geschwächter Schutz vermerkt
    # (klassisches Beispiel: Jupiter unter Ketu = »falscher Optimismus«).
    def _prot_weak(p):
        """Schwächungen eines Schutzplaneten: eigene Affliktionen ODER
        Übeltäter-Einfluss auf sein Haus (Besetzung/Drishti — z.B. Ketu-Aspekt
        oder Saturn-Konjunktion auf Jupiter)."""
        out = [a for a in aff.get(p, [])
               if any(k in a.lower() for k in
                      ("debil", "combust", "nodal", "grahana", "yuddha"))]
        _phh = h(p)
        if _phh:
            _mal_infl = [q for q in _influences_on_house(chart, _phh)
                         if q != p and q in _MALEFICS]
            if _mal_infl:
                out.append("Übeltäter-Einfluss: "
                           + ", ".join(DE[q] for q in _mal_infl))
        return out
    _bhanga: List[str] = []
    _weak_prot: List[str] = []
    _moon_si_ = pls.get("Moon", {}).get("sign_idx")
    for _prot in ("Jupiter", "Venus"):
        _ph = h(_prot)
        _where = None
        if _ph in _KENDRA:
            _where = f"{DE[_prot]} im Kendra vom Lagna"
        elif _moon_si_ is not None and pls.get(_prot, {}).get("sign_idx") is not None:
            _hm = (pls[_prot]["sign_idx"] - _moon_si_) % 12 + 1
            if _hm in _KENDRA:
                _where = f"{DE[_prot]} im Kendra vom Mond"
        if _where:
            _w = _prot_weak(_prot)
            if _w:
                _weak_prot.append(f"{_where} — aber selbst affligiert ({_w[0]})")
            else:
                _bhanga.append(_where)
    _m_ben_ok = [p for p in _m_ben if not _prot_weak(p)]
    if _m_ben_ok:
        _bhanga.append(f"Wohltäter-Einfluss auf den Mond "
                       f"({', '.join(DE[p] for p in _m_ben_ok)})")
    if dig(lagna_lord) in ("Exalted", "Own Sign", "Moolatrikona")             and not _prot_weak(lagna_lord):
        _bhanga.append(f"starker Lagna-Herr ({DE.get(lagna_lord)}: {dig(lagna_lord)})")
    if _bal_find:
        _txt = "; ".join(_bal_find)
        if _bhanga:
            _txt += " — AUFGEHOBEN/gemildert (Ariṣṭa-Bhaṅga): " + "; ".join(_bhanga)
        if _weak_prot:
            _txt += " — nur geschwächter Schutz: " + "; ".join(_weak_prot)
        _txt += " · retrospektiv: betrifft die frühen Kindheitsjahre"
        row("Balarishta (Kindheits-Arishta)", _txt, "·" if _bhanga else "−")
    else:
        row("Balarishta (Kindheits-Arishta)",
            "keine klassische Kombination (Mond ausreichend geschützt)", "·")
    ...
    # 6. Achtes Haus (Āyus-Haus). Klassische Gewichtung:
    #    + starker/würdevoller H8-Herr STÜTZT die Lebensspanne (Exaltation,
    #      eigenes Zeichen, Moolatrikona) — das 8. ist selbst das Āyus-Haus;
    #    − H8-Herr in 7 oder 12, Debilitation oder Affliktionen;
    #    · "stärker als der Lagna-Herr" ist ein VORBEHALT in der Mischung
    #      ("im Idealfall nicht stärker"), KEIN Veto — er darf eine
    #      Exaltation nicht ins Minus kippen.
    h8_lord = SIGN_LORDS.get(houses.get(8, houses.get("8")))
    l8 = h(h8_lord) if h8_lord else 0
    r8, rl = rupa(h8_lord), rupa(lagna_lord)
    a8 = aff.get(h8_lord, [])
    strong_dig = dig(h8_lord) in ("Exalted", "Own Sign", "Moolatrikona")
    bad_pos = l8 in (7, 12)
    findings = [f"H8-Herr {DE.get(h8_lord)} in Haus {l8}, Würde: {dig(h8_lord)}"]
    if strong_dig:
        findings.append("würdevoller 8.-Herr stützt das Āyus-Haus")
    if bad_pos:
        findings.append("Stand in 7/12 — klassisch ungünstig")
    if "Debil" in dig(h8_lord):
        findings.append("debilitiert")
    if a8:
        findings.append(f"Affliktionen: {'; '.join(a8)}")
    if r8 is not None and rl is not None:
        findings.append(f"Rupas H8-Herr {r8} vs. Lagna-Herr {rl}")
        if r8 > rl:
            findings.append("Vorbehalt: stärker als der Lagna-Herr "
                            "(im Idealfall umgekehrt)")
    # Nur SCHWERE Affliktionen blockieren das Würde-Plus (Verbrennung,
    # Knoten-Konjunktion, Gandanta/Sandhi, Graha Yuddha) — Papakartari oder
    # Rückläufigkeit sind vermerkt, kippen aber einen exaltierten Āyus-Herrn
    # nicht (Rückläufigkeit stärkt die Lebenslänge sogar).
    serious8 = [a for a in a8 if any(k in a.lower() for k in
                ("combust", "nodal", "grahana", "gandanta", "sandhi", "yuddha"))]
    if strong_dig and not bad_pos and not serious8:
        m = "+"
    elif (bad_pos or "Debil" in dig(h8_lord) or serious8) and not strong_dig:
        m = "−"
    else:
        m = "·"
...
def render_tab(chart: Dict) -> str:
    ...
    if focus["items"]:
        ...
        for it in focus["items"]:
            conv = ("<br><span style='color:var(--ac);font-size:.76rem'>"
                    "&#9679; Konvergenz beider Ebenen &mdash; verst&auml;rkt"
                    "</span>") if it["convergence"] else ""
            rows_f += (
                f"<tr><td>{it['body']}<br><span style='color:var(--mu);"
                f"font-size:.78rem'>Region {it['region']} &middot; "
                f"{it['sign']}</span>{conv}</td>"
                f"<td>Haus {it['region']} ({it['house_part']}): "
                f"{', '.join(it['house_who']) or '&mdash;'}<br>"
                f"<span style='color:var(--mu)'>{it['sign']} als Zeichen "
                f"({it['sign_part']}): {', '.join(it['sign_who']) or '&mdash;'}"
                f"</span></td>"
                f"<td style='text-align:center'>{it['score']}</td></tr>")
        focus_html += (
            f"<p style='font-size:.84rem;color:var(--mu);line-height:1.5'>"
            f"Zweischichtige Zuordnung nach der K&#257;lapuru&#7779;a-Systematik: "
            f"KAP-gewichtete Belastung je <strong>Haus</strong> (vom Lagna) und "
            f"je <strong>Zeichen ab Widder</strong> (fixe Melothesie), "
            f"zusammengef&uuml;hrt &uuml;ber die zw&ouml;lf K&ouml;rperregionen "
            f"(Besetzung voll, Aspekte halb). Tragen beide Ebenen zur selben "
            f"Region bei, gilt der Bereich als verst&auml;rkt &mdash; als "
            f"Einladung zu Achtsamkeit und Vorsorge, nicht als Befund.</p>"
            f"<div class='ow'><table class='dt'><thead><tr>"
            f"<th>K&ouml;rperregion</th><th>Beitr&auml;ge (Haus- / Zeichen-Ebene)"
            f"</th><th>Punkte</th></tr></thead>"
