...
def _build_aspects(planets: Dict, lagna_idx: int) -> Dict:
    """Build Graha Drishti with benefic/malefic tagging and house-level summaries."""
    ...
    # Track benefic/malefic aspects per house for house-level summary
    ...
    for p1, h1 in planet_houses.items():
        ...
        is_benefic = p1 in _BENEFICS
        is_malefic = p1 in _MALEFICS
        tag = "BENEFIC-aspect" if is_benefic else "MALEFIC-aspect"
        ...
        for p2, h2 in planet_houses.items():
            ...
            if h2 in house_to_aspnum:
                ...
                entry.append(f"{tag} {asp_num}th on {p2} (H{h1}→H{h2})")
                aspects[p1] = entry
                recv = aspects.get(p2, [])
                recv.append(f"receives {tag} {asp_num}th from {p1} (H{h1}) → {'strengthens' if is_benefic else 'afflicts'} {p2}")
                aspects[p2] = recv

        # Also record aspects on empty houses
        for asp_h, asp_num in house_to_aspnum.items():
            if is_benefic:
                house_benefic[asp_h].append(f"{p1}({asp_num}th)")
            else:
                house_malefic[asp_h].append(f"{p1}({asp_num}th)")

    # Add house-level aspect summary
    aspects["_house_aspects"] = {
        h: {
            "benefic": house_benefic[h],
            "malefic": house_malefic[h],
            "net": ("strengthened" if house_benefic[h] and not house_malefic[h]
                    else "afflicted" if house_malefic[h] and not house_benefic[h]
                    else "mixed" if house_benefic[h] and house_malefic[h]
                    else "neutral")
        }
        for h in range(1, 13)
        if house_benefic[h] or house_malefic[h]
    }
...
4. ASPEKTE: Die Aspekte im JSON enthalten explizit die Aspekt-Nummer UND den Typ (BENEFIC-aspect / MALEFIC-aspect). Verwende IMMER diese Tags — berechne niemals selbst welcher Aspekt es ist. BENEFIC-aspect (Jupiter, Venus, Mond, Merkur) stärkt den aspektierten Planeten oder das Haus. MALEFIC-aspect (Saturn, Mars, Sonne, Rahu, Ketu) affliktiert ihn. Gemischte Aspekte (gleichzeitig Benefic + Malefic) zeigen gemischte Ergebnisse. Das Feld "_house_aspects" gibt für jedes Haus eine Netto-Einschätzung (strengthened / afflicted / mixed / neutral).
...
4. ASPECTS: The aspects JSON contains explicit aspect numbers AND type tags (BENEFIC-aspect / MALEFIC-aspect). Always use these — never calculate aspect numbers yourself. BENEFIC-aspect (Jupiter, Venus, Moon, Mercury) strengthens the aspected planet or house. MALEFIC-aspect (Saturn, Mars, Sun, Rahu, Ketu) afflicts it. Mixed aspects (both benefic and malefic on same planet/house) produce mixed results. The "_house_aspects" field gives a net assessment per house (strengthened / afflicted / mixed / neutral).
