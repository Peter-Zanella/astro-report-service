...
def build_facts(chart: Dict, depth: str = "premium") -> str:
    ...
    # ── Afflictions ───────────────────────────────────────────────────────
    # Knoten-Verbindung eines Planeten (Regel 15) — reines ABLESEN aus dem
    # Engine-Produkt chart["conjunctions"]; ai_report misst keine Abstände.
    _conj_all = chart.get("conjunctions", {}) or {}

    def _node_link(_pl):
        for _e in _conj_all.get(_pl, []):
            if _e.get("with") in ("Rahu", "Ketu") and _e.get("orb", 99) <= 8.0:
                return {"node": _e["with"], "orb": round(_e["orb"], 1),
                        "tight": _e["orb"] < 3.0}
        return None
    ...
    if houses_map:
        ...
        for h in range(1, 13):
            ...
            entry = {
                ...
                # unübersehbares Flag: Hausherr im Dushthana belastet die
                # Themen seines Hauses (Herr von 2 in 12 → Besitz-Abfluss usw.)
                "in_dushthana": rec.get("house") in (6, 8, 12),
                # Rahu/Ketu-Signatur des Hausherrn (Regel 15): enge Knoten-
                # Verbindung färbt die Hausthemen (Rahu: Ausland/unkonventionell,
                # Ketu: Ablösung/Spiritualisierung)
                "node_link": _node_link(lord),
    ...
    if meta.get("questions"):
        f["meta"]["questions"] = [q for q in meta["questions"] if q][:3]
    # Prāśna question (if set)
    if chart.get("prasna_question"):
        f["prasna_question"] = chart["prasna_question"]
        f["prasna_mode"] = True
        # Gulika/Māndi als klassische Prāśna-Bestätigungsfaktoren — vorberechnet
        # inkl. Haus vom Prāśna-Lagna UND vom Mond (Rechenverbot gilt auch hier).
        _upa = chart.get("upagrahas", {}) or {}
        _gu = {}
        for _uk, _ulabel in (("Gulika", "gulika"), ("Mandi", "mandi")):
            _ur = _upa.get(_uk)
            if not _ur:
                continue
            _entry = {"sign": _ur.get("sign"), "pos": _ur.get("pos"),
                      "house": _ur.get("house"),
                      "nakshatra": f"{_ur.get('nakshatra','')} Pada {_ur.get('pada','')}"}
            if _moon_si is not None and _ur.get("sign_idx") is not None:
                _entry["house_from_moon"] = (_ur["sign_idx"] - _moon_si) % 12 + 1
            _gu[_ulabel] = _entry
        if _gu:
            f["upagrahas"] = _gu
...
1. GEWICHTUNG: Ein Planet ist stärker wenn er (a) hohe Shad-Bala-Rupas hat, (b) in einem Kendra/Trikona steht, (c) im eigenen Zeichen oder erhöht ist. Schwächere Planeten werden entsprechend relativiert. Das Feld 'dignity' ist AUCH für Freund- und Feindzeichen autoritativ (Great Friend's/Friend's/Neutral/Enemy's/Great Enemy's Sign — zusammengesetzte Freundschaft): Behaupte NIEMALS aus eigenem Wissen, ein Zeichen sei einem Planeten zuträglich oder unzuträglich — verwende ausschliesslich dieses Feld. 'Neutral Sign' ist neutral und wird NICHT als Schwäche gedeutet.
...
8. GULIKA (Prāśna-Bestätigungsfaktor): 'upagrahas.gulika' (und 'mandi') stehen mit 'house' (vom Prāśna-Lagna) und 'house_from_moon' FERTIG berechnet bereit — niemals selbst zählen. Klassische Regel (Prashna Marga): Gulika im 1. Haus, beim Mond ('house_from_moon' = 1) oder im Fragehaus warnt vor verborgenen Störungen, Täuschung oder einem belasteten Fragehintergrund und MUSS dann benannt und im Urteil gewichtet werden. Gulika in 6/8/12 des Prāśna-Charts ist für die Frage tendenziell entlastend (das Störende ist »weggesperrt«). Berührt Gulika keinen relevanten Punkt, erwähne ihn höchstens beiläufig — kein Drama um einen unauffälligen Gulika.""",
...
8. GULIKA (praśna confirmation factor): 'upagrahas.gulika' (and 'mandi') come PRE-COMPUTED with 'house' (from the praśna Lagna) and 'house_from_moon' — never count yourself. Classical rule (Prashna Marga): Gulika in the 1st house, with the Moon ('house_from_moon' = 1) or in the question's house warns of hidden disturbances, deception or a burdened background and MUST then be named and weighed in the verdict. Gulika in 6/8/12 of the praśna chart tends to be relieving for the question (the disturbing element is "locked away"). If Gulika touches no relevant point, mention it at most in passing — no drama around an unremarkable Gulika.""",
...
1. WEIGHTING: A planet is stronger when it has (a) high Shad Bala rupas, (b) is in a Kendra/Trikona, (c) is in its own sign or exalted. Qualify weaker planets accordingly. The 'dignity' field is ALSO authoritative for friend and enemy signs (Great Friend's/Friend's/Neutral/Enemy's/Great Enemy's Sign — compound friendship): NEVER assert from your own knowledge that a sign is favourable or unfavourable for a planet — use only this field. 'Neutral Sign' is neutral and is NOT interpreted as weakness.
...
_SECTION_GUIDES = {
    "de": {
        ...
        "Daśāṃśa (D10) — Beruf & Karriere":
            ...
            "Prüfe 'house_lords.lords.house_10.node_link': Rahu-Verbindung des "
            "10.-Herrn → Beruf im/mit dem Ausland oder aussergewöhnliche "
            "Tätigkeit (Regel 15). "
        ...
        "Beziehungen & Partnerschaft":
            ...
            "Aspekte auf das 7. Haus). Prüfe 'house_lords.lords.house_7.node_link': "
            "Rahu-Verbindung des 7.-Herrn → Partner/in aus dem Ausland oder "
            "anderem Kulturkreis bzw. unkonventionelle Partnerschaft (Regel 15). "
            "Venus als natürlicher Karaka. "
        ...
        "Lagna & Lagna-Herr — Der Fragesteller":
            ...
            "Prüfe zudem 'upagrahas.gulika': steht Gulika im 1. Haus oder bei "
            "Lagna-Herr/Mond, benenne die klassische Warnung vor verborgenen "
            "Störungen. "
        ...
        "Antwort auf deine Frage":
            "Beantworte die Frage aus 'meta.questions' gründlich aus dem "
            "GEBURTSHOROSKOP heraus (kein Prāśna): ziehe die thematisch "
            "passenden Häuser, ihre Herren, Kārakas, Divisionalcharts und die "
            "aktuelle Daśā-Zeit heran ('vimshottari_current', 'transits_today' "
    ...
    "en": {
        ...
        "Lagna & Lagna lord — The querent":
            ...
            "Also check 'upagrahas.gulika': if Gulika sits in the 1st house or "
            "with the Lagna lord/Moon, name the classical warning of hidden "
            "disturbances. "
        ...
        "Answer to your question":
            "Answer the question in 'meta.questions' thoroughly from the "
            "NATAL chart (not a praśna): draw on the thematically fitting "
            "houses, their lords, karakas, divisional charts and the current "
            "dasha period ('vimshottari_current', 'transits_today' for "
...
def build_prompt(facts: str, lang: str, depth: str) -> str:
    secs = list(_SECTIONS[lang][depth])
    guides = dict(_SECTION_GUIDES[lang])   # lokale Kopie — nie global mutieren

    # Individuelle Kundenfragen (bis 3) im Haupt-Reading: eigenes Kapitel am
    # Ende; die allgemeinen Varga-/Beziehungskapitel werden dafür kompakter.
    q_extra = ""
    try:
        _qs = (json.loads(facts).get("meta", {}) or {}).get("questions", []) or []
    except Exception:
        _qs = []
    if _qs and depth in ("basis", "premium", "year"):
        if lang == "de":
            q_title = "Deine Fragen an dein Horoskop"
            guides[q_title] = (
                "Beantworte JEDE der folgenden Fragen einzeln und gründlich "
                "aus dem Geburtshoroskop (je 250–450 Wörter, mit eigener "
                "### Zwischenüberschrift pro Frage): "
                + " | ".join(f"Frage {i+1}: «{q}»" for i, q in enumerate(_qs))
            ...
            q_title = "Your questions to your chart"
            guides[q_title] = (
                "Answer EACH of the following questions individually and "
                "thoroughly from the natal chart (250–450 words each, with "
                "its own ### subheading per question): "
    ...
    if lang == "de":
        head = ("Unten stehen die exakt berechneten Horoskop-Fakten — dein INTERNES "
                "Arbeitsmaterial. Der Leser darf von seiner Existenz NICHTS erfahren: "
                "Erwähne im Bericht niemals JSON, Tabellen, Felder, Datensätze, "
                "'gelieferte Fakten' oder Feldnamen. Schreibe, als läsest du direkt "
                "im Horoskop ('In deinem Horoskop steht Saturn …', 'Dein Mond …'). "
                ...
                "auch die Anweisungen und ihre Feldverweise sind intern und werden "
                "nie zitiert.\n\n")
    else:
        head = ("Below are the precisely calculated chart facts — your INTERNAL "
                "working material. The reader must learn NOTHING of its existence: "
                "never mention JSON, tables, fields, records, 'supplied facts' or "
                "field names in the report. Write as if reading the horoscope "
                "directly ('In your chart, Saturn stands …', 'Your Moon …'). "
                ...
                "but write fluidly and personally, NOT like a checklist; the "
                "instructions and their field references are internal too and are "
                "never quoted.\n\n")
    ...
    return (f"{head}{q_extra}{sections_txt}\n\n--- INTERNE FAKTEN (niemals erwähnen "
            f"oder zitieren) ---\n{facts}")
