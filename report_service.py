...
<form method="post" action="/generate" onsubmit="var b=this.querySelector('button[type=submit]');b.disabled=true;b.textContent='⏳ Bericht wird erstellt…';document.getElementById('working').style.display='block';">
...
<form method="post" action="/prasna-generate"
  onsubmit="var b=this.querySelector('button[type=submit]');b.disabled=true;
  b.textContent='⏳ Prāśna wird berechnet…';document.getElementById('pw').style.display='block';">
...
def prasna_page():
    return HTMLResponse(_prasna_form_html())
...
def prasna_generate(
    ...
    sid = "PRASNA-" + _dt.datetime.utcnow().strftime("%Y%m%d%H%M%S%f")
    fn = f"Prasna_{y}{mo:02d}{d:02d}.pdf"
    _PDF_CACHE[sid]  = (pdf, fn)
    _HTML_CACHE[sid] = html_view
    return RedirectResponse(url=f"/view/{sid}", status_code=303)
