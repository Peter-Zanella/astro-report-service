...
def get_paid_session(session_id: str):
    """Return (ok, info) where info has email, name, depth, label. Verifies payment."""
    if DEV_NO_STRIPE:
        return True, {"email": "", "name": "", "depth": "premium", "label": "Bericht (DEV)"}
    try:
        stripe = _stripe()
...
def report_form(session_id: str = ""):
    if not session_id:
    ...
    ok, info = get_paid_session(session_id)
    if not ok:
...
def generate(session_id: str = Form(...), name: str = Form(""), date: str = Form(...),
    ...
    try:
        send_pdf_email(email, name, pdf, info["label"])
    except Exception:
        pass  # download still works even if e-mail fails
