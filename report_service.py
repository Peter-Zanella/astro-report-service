...
def get_paid_session(session_id: str, want_depth: str = ""):
    ...
    if TEST_MODE:
        depth = want_depth if want_depth in ("basis", "premium", "prasna") else "premium"
        label = f"{depth.capitalize()}-Bericht (TEST)" if depth != "prasna" else "Prāśna (TEST)"
...
def varshaphala_age(session_id: str, age: int):
    """Recompute Varshaphala (annual chart) for a given age. Called via AJAX."""
    ...
    try:
        ...
        vp = E.compute_varshaphala(
            params["birth_year"], params["mo"], params["d"],
            params["natal_sun_sid"], params["natal_lagna_si"],
            params["lat"], params["lon"], target_year,
        )
        ...
        return JSONResponse({
            ...
            "planets": {
                p: {
                    "sign":      d.get("sign"),
                    "sign_idx":  d.get("sign_idx"),
                    "house":     d.get("house"),
                    "pos":       d.get("pos"),
                    "nakshatra": d.get("nakshatra"),
                    "dignity":   d.get("dignity"),
                }
                for p, d in vp.get("planets", {}).items()
            },
        })
    except Exception as e:
        import traceback, sys
