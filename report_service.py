...
# ── in-memory caches (reset on restart — fine for short-lived sessions) ────────
_PDF_CACHE:   dict = {}
_HTML_CACHE:  dict = {}   # HTML view per session
_CHART_CACHE: dict = {}   # raw birth params for Varshaphala recompute
...
def download_pdf(session_id: str):
    ...
    return StreamingResponse(BytesIO(pdf), media_type="application/pdf",
        ...
        _CHART_CACHE[session_id] = {"year":y,"month":mo,"day":d,"hour":hh,"minute":mm,"lat":loc["lat"],"lon":loc["lon"],"offset":loc["offset"],"name":name,"gender":gender,"location":location}
...
def view_chart(session_id: str):
    ...
    if not html:
        return HTMLResponse(msg_html("Ansicht abgelaufen",
                            "Die interaktive Ansicht ist nicht mehr verfügbar.", "err"), 404)
    return HTMLResponse(html)


@app.get("/varshaphala/{session_id}/{age}")
def get_varshaphala(session_id: str, age: int):
    from fastapi.responses import JSONResponse
    bp = _CHART_CACHE.get(session_id)
    if not bp:
        return JSONResponse({"error": "Session not found or expired."}, status_code=404)
    if not (1 <= age <= 100):
        return JSONResponse({"error": "Age must be 1-100."}, status_code=400)
    try:
        target_year = bp["year"] + age
        natal = E.generate_chart(
            bp["year"], bp["month"], bp["day"],
            bp["hour"], bp["minute"],
            bp["lat"], bp["lon"], bp["offset"],
            bp["location"], bp["name"], bp.get("gender","")
        )
        vp = E.compute_varshaphala(
            bp["year"], bp["month"], bp["day"],
            natal["planets"]["Sun"].get("lon_sid", 0),
            natal["lagna_idx"],
            bp["lat"], bp["lon"],
            target_year
        )
        result = {
            ...
            "sarva": vp.get("ashtakavarga",{}).get("Sarva",[]),
        }
