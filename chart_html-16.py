...
def build_html(chart:Dict, interpretation:Optional[str]=None,
    ...
    if eclipse_db and eclipse_db.available():
        ...
        eclipse_html = (
            ...
            f"<div id='ecl-scroll' style='max-height:600px;overflow-y:auto;border:1px solid var(--bd);border-radius:8px'>"
...
<div class="tp" id="tab-varsha">
  ...
  <div id="vp-content">
    ...
    <table class="ptab" style="margin-top:18px"><thead><tr>
      <th>Planet</th><th>Zeichen</th><th>Haus</th><th>Grad</th><th>Nakshatra</th><th>Würde</th>
    </tr></thead><tbody id="vp-planet-rows">{vp_planet_rows}</tbody></table>
  </div>
</div>
...
<div class="tp" id="tab-compat">
  ...
  <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:end;margin-bottom:20px">
    <div style="display:flex;flex-direction:column;gap:4px">
      <label style="font-size:.78rem;color:var(--mu)">Geburtsdatum (TT.MM.JJJJ)</label>
      <input id="cp-date" type="text" placeholder="15.06.1988"
        style="padding:9px 12px;background:var(--bg2);color:var(--tx);border:1px solid var(--bd);border-radius:6px;font-family:inherit;font-size:.9rem;width:150px">
    </div>
    <div style="display:flex;flex-direction:column;gap:4px">
      <label style="font-size:.78rem;color:var(--mu)">Uhrzeit (optional)</label>
      <input id="cp-time" type="text" placeholder="12:00" value="12:00"
...
function renderCompat(d){{
  ...
  h+='</div>';
  // Kuta table
  h+='<table class="ptab" style="width:100%;border-collapse:collapse;margin-bottom:20px"><thead><tr><th>Kūṭa</th><th>Punkte</th><th style="width:40%">Bedeutung</th></tr></thead><tbody>';
  ...
  h+='<div style="color:var(--mu);font-size:.8rem;margin-top:6px">Person A: Mars im '+m.a.house_lagna+'. Haus (ab Lagna)'+(m.a.present?' — Dosha':' — kein Dosha')+' &nbsp;·&nbsp; Person B: Mars im '+m.b.house_lagna+'. Haus'+(m.b.present?' — Dosha':' — kein Dosha')+'</div>';
  h+='</div>';
  // House overlay
