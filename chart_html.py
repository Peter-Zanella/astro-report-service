...
def build_html(chart:Dict, interpretation:Optional[str]=None,
    ...
    for md in dashas.get("mahadashas",[]):
        ...
        cls=" act" if a else ""
        now=" (now)" if a else ""
        maha_rows+=f"<tr class='{cls}'><td>{md.get('planet','')}{now}</td><td>{md.get('start','')}</td><td>{md.get('end','')}</td><td>{md.get('years','')} yrs</td></tr>"
    ...
    for md in dashas.get("mahadashas",[]):
        if md.get("active"):
            for ad in md.get("antardashas",[]):
                ...
                cls=" act" if a else ""
                now=" (now)" if a else ""
                antar_rows+=f"<tr class='{cls}'><td>{md.get('planet','')}/{ad.get('planet','')}{now}</td><td>{ad.get('start','')}</td><td>{ad.get('end','')}</td></tr>"

    # ── Chara Dasha ───────────────────────────────────────────────────────────
    chara=chart.get("chara_dasha",{})
    chara_cur=chara.get("current",{})
    chara_rows=""
    for rd in chara.get("rasi_dashas",[]):
        a=rd.get("active",False)
        cls=" act" if a else ""
        now=" (now)" if a else ""
        chara_rows+=f"<tr class='{cls}'><td>{rd.get('sign','')}{now}</td><td>{rd.get('start','')}</td><td>{rd.get('end','')}</td><td>{rd.get('years','')} yrs</td></tr>"
    ...
    for rd in chara.get("rasi_dashas",[]):
        if rd.get("active"):
            for ad in rd.get("antardashas",[]):
                ...
                cls=" act" if a else ""
                now=" (now)" if a else ""
                chara_antar_rows+=f"<tr class='{cls}'><td>{rd.get('sign','')}/{ad.get('sign','')}{now}</td><td>{ad.get('start','')}</td><td>{ad.get('end','')}</td></tr>"
