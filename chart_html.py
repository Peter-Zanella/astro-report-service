...
def build_html(chart:Dict, interpretation:Optional[str]=None,
    ...
    chara_cmp_rows = ""
    for _sn in SIGNS:
        _a, _b = _d_nr.get(_sn), _d_pa.get(_sn)
        if _a is None or _b is None:
            continue
        _diff = _a != _b
        _st = " style='color:var(--ac);font-weight:600'" if _diff else ""
        chara_cmp_rows += (f"<tr><td>{_sn}</td><td>{_a} yrs</td>"
                           f"<td{_st}>{_b} yrs</td>"
                           f"<td style='color:var(--mu)'>{'abweichend' if _diff else ''}</td></tr>")
    chara_p_rows = ""
    for rd in chara_p.get("mahadashas", []):
        a = rd.get("active", False)
        cls = " act" if a else ""
        now = " (now)" if a else ""
        s_ = str(rd.get('start', ''))[:10]; e_ = str(rd.get('end', ''))[:10]
        chara_p_rows += (f"<tr class='{cls}'><td>{rd.get('sign','')}{now}</td>"
                         f"<td>{s_}</td><td>{e_}</td><td>{rd.get('years','')} yrs</td></tr>")
    chara_cur_pada = chara_p.get("current") or "—"
    ...
    try:
        ...
        remedies_html = _rem.render_tab(chart)
    except Exception as _re:
        remedies_html = ("<p style='color:var(--mu)'>Remedies-Modul nicht "
                         f"verf&uuml;gbar ({type(_re).__name__}).</p>")
...
<div class="tp" id="tab-chara">
  ...
  sie unterscheiden sich in genau den vier fixen Zeichen. Die Richtung wird
  an <em>zwei</em> Stellen gebraucht: für die Daśā-<strong>Folge</strong> und
  für die <strong>Dauer</strong>-Zählung je Zeichen zu seinem Herrn. Für die
  Folge ist dabei nicht das Lagna massgeblich, sondern das <strong>9. Zeichen
  vom Lagna</strong> — bei Skorpion-Lagna also Krebs, das rückwärts läuft.
  Diese 9.-Zeichen-Regel gilt ausdrücklich <em>nicht</em> für die Dauern.</p>
  <div class="ow"><table class="dt">
  ...
  (Jaimini-Sūtra-Tradition). Ihr folgen hier Folge <em>und</em> Dauern.<br>
  ...
  Die Tabelle unten stellt beide Dauern gegenüber. Laufendes Zeichen:
  <strong>{chara_cur}</strong> nach der Pada-Regel,
...
<div class="tp" id="tab-remedies">
  <p class="sh">Upāyas · Klassische Remedies</p>
  {remedies_html}
</div>
